...
def patch_flow4(zip_path: Path, *, action_name: str) -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        if action_name not in actions:
            print(f"INFO: action '{action_name}' not found; nothing to patch.")
            return

        # Remove Teams notification action to prevent channel/user noise during test resets.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch Flow4 export zip to remove Teams channel notifications.")
    parser.add_argument("--zip", required=True, help="Path to flow4.zip")
    parser.add_argument(
        "--action-name",
        default="Postar_cartão_em_um_chat_ou_canal",
        help="Action name to remove (default matches current Flow4 export).",
    )
    args = parser.parse_args()
    patch_flow4(Path(args.zip), action_name=args.action_name)
    print(f"OK: patched {args.zip} (removed Teams notify action: {args.action_name})")
    return 0
